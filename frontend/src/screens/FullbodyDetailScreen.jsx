...
const FullbodyDetailScreen = () => {
  ...
  const [fullbodys, setFullbody] = useState({});
  const { id: productId } = useParams();

  useEffect(() => {
    const fetchFullbody = async () => {
      const {data} = await axios.get(`/api/fullbody/${productId}`);
      setFullbody(data);
    };
    fetchFullbody();
  }, [productId]);
      ...
      <div className="product-container box-shadow">
        <div className="top-section flex">
          ...
          <div className="top-content">
            <p className="fs-400">{fullbodys.alt}</p>
